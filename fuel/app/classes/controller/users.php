...
*/
					...
					if($valid["twit_connect"] == "on"){
						try {  
							...
							$gamelist = $this->arr_game();
                                                        ...
                                                        //店舗名
                                                        $tenponame = \Model_Gamecentertest::find_by_tenponame($valid["checkin_tenpo"]);
                                                        ...
                                                        if($valid["showtime"] == "on"){
                        					$msg = $twitter->statuses->update($data["tenpo_name"]."(".$valid["user_date_m"]."月".$valid["user_date_d"]."日 ".$valid["user_time_h"].":".$valid["user_time_m"]."着予定) ".$valid['comment']." #ゲー行く_".$gamelist[$valid["game"]]." http://bit.ly/Yo64Uj"); 
                                                        }else{
                           					$msg = $twitter->statuses->update($data["tenpo_name"]." ".$valid['comment']." #ゲー行く_".$gamelist[$valid["game"]]." http://bit.ly/Yo64Uj");                                                          
                                                        }
	...
	public static function first_statas($user_id){
		...
		$data['fav_tenpo_game'] = $account->fav_tenpo_game;
        		...
        		$tenponame = \Model_Gamecenter::find_by_tenponame($account->fav_tenpo);
                        $data["tenpo_name"] = $tenponame->name;
                }else{
                    $data["tenpo_name"] = "";
                }
