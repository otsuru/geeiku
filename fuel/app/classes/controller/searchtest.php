 ...
 * testfile
 */
class Controller_Searchtest extends Controller_Template
{	
	...
	{			
		...
		$this->template->content = View::forge('searchtest/index',$data);
	...
	{			
		...
		$this->template->header = View::forge('default/header');
		$this->template->footer = View::forge('default/footer');
		$this->template->content = View::forge('searchtest/index');
		$select_list = \Model_Gamecentertest::set_array_as_mst();
                $this->template->content->set('select_list', $select_list);
	...
	{	
		...
		if (!$val->run())
		{
			...
			$this->template->header = View::forge('default/header');
			$this->template->footer = View::forge('default/footer');
			$this->template->content = \View::forge('searchtest/index', $this->_data);
		...
		else
		{
			...
			if($valid["keyword"] != "" && is_numeric($valid["game"])){
				...
				if(isset($userdata["0"]["tenpo_id"])){
					$tenponame = \Model_Gamecentertest::find_by_tenponame($userdata["0"]["tenpo_id"]);
					if(!is_null($tenponame)){
						$tenponame = $tenponame->name;
					}else{
						$tenponame = 0;
			...
			elseif(is_numeric($valid["tenpo"]) && is_numeric($valid["game"])){
				$tenponame = \Model_Gamecentertest::find_by_tenponame($valid["tenpo"]);
				$tenponame = $tenponame->name;
				$this->template->content->set('tenponame', $tenponame);
