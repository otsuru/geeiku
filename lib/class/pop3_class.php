...
[ Constractor ]
    ...
    pop3(string server, string user, string pass [, int port = 110] [, bool ssl = FALSE])
...
class pop3
{
	...
	var $ssl;       // use SSL Add by.koba@NP 2009-03-02

	/* Private variables */
	var $_sock;

	//function pop3($server, $user, $pass, $port = 110)
	function pop3($server, $user, $pass, $port = 110, $ssl = FALSE)	// Add by.koba@NP 2009-03-02
	{
		$this->apop     = FALSE;
//		$this->debug    = FALSE;
		$this->debug    = TRUE;
		...
		$this->user     = $user;
		$this->pass     = $pass;
		$this->ssl      = $ssl;	// Add by.koba@NP 2009-03-02
	...
	function open()
	{
		...
		$this->_sock = @fsockopen(($this->ssl ? "ssl://" : "").$this->server, $this->port, $errno, $errstr, $this->time_out);	// Add by.koba@NP 2009-03-02
		if (!$this->_sock) {
	...
	//* Add By.Koba 2007-06-06
	function parsehead($num)
	{
		$headers = array();
		
		$raw_headers = rtrim($this->head($num));
