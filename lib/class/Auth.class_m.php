	...
	function __construct(&$objForm)
	{
		...
		if ($this->objForm->get("auth_flg") == "1"){
			...
			// えらーなし
				...
				$url = "/mob/?no=".$this->objForm->get("no");
				//	."&manage_no=".urlencode($objForm->get("manage_no"))
				//	."&manage_name=".urlencode($objForm->get("manage_name"))
				//	."&manage_company=".urlencode($objForm->get("manage_company"));
					
				$_SESSION["manage_no"] = ch_utf($objForm->get("manage_no"));
				$_SESSION["manage_name"] = ch_utf($objForm->get("manage_name"));
				$_SESSION["manage_company"] = ch_utf($objForm->get("manage_company"));
				
				header("Location:".$url."&".session_name() . '=' . strip_tags(session_id()));
			}
		}
		// ログインフォーム以外、通常アクセス時
		else {
			...
			elseif ($this->objForm->get("logout") != ""){
				$this->viewLoginForm();
				//header("Location: ./?no=".$this->objForm->get("no")."&".session_name() . '=' . strip_tags(session_id()));
			}
/*
			// ログイン情報なし時
...
*/
	...
	function viewLoginForm($strMsg = "")
	{
		...
		include CNF_HTML_PATH.'mob/login.html';
